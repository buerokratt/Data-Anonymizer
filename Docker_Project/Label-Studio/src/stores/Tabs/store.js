...
export const TabStore = types
  ...
  .actions((self) => ({
    ...
    saveView: flow(function* (view, { reload, interaction } = {}) {
      ...
      const result = yield Promise.resolve({ "id":1, "data": body.body.data });
    ...
    fetchColumns() {
      ...
      columns.forEach((col) => {
        ...
        if (!col.children && column.filterable && (['tasks:id', 'tasks:completed_at', 'tasks:data.text'].includes(column.id))) {
    ...
    fetchTabs: flow(function* (tab, taskID, labeling) {
      ...
      const response = [{ "id": 1,"data":{ "type":"list","title":"Default","target":"tasks","gridWidth":4,"columnsWidth":{},"hiddenColumns":{ "explore":[],"labeling":["tasks:id","tasks:completed_at"] },"columnsDisplayType":{},"ordering":["-tasks:id"] } }];
