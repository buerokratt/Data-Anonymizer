  ...
  return DataStore("TasksStore", {
    ...
    .actions((self) => ({
      loadTask: flow(function* (taskID, { select = true } = {}) {
        ...
        let taskRecord = yield self.root.apiCall("task", { id: taskID });
        let taskData = {
          data: { text: "" },
          predictions: [],
          annotations: [{}],
        ...
        taskData["id"] = taskID;
        taskData["data"]["text"] = taskRecord["rawText"];
        taskData["annotations"][0]["result"] = taskRecord["predictions"]
          ? JSON.parse(taskRecord["predictions"])
          : [];
        taskData["annotations"][0]["task"] = taskID;
        let task = self.applyTaskSnapshot(taskData, taskID);
