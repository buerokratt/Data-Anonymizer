...
export class LSFWrapper {
  ...
  /** @private */
  async preloadTask() {
    const { annotation: annotationId, draft: draftId } = this.preload;
    ...
    if (response && response.task) {
      const task = await api.call("task", {
        params: { id: response.task },
      });
  ...
  prepareData(annotation, { includeId, draft } = {}) {
    ...
    const result = {
      ...
      lead_time:
        (new Date() - annotation.loadedDate) / 1000 +
        Number(annotation.leadTime ?? 0),
